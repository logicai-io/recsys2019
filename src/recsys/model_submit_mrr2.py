...
import numpy as np
import pandas as pd
from lightgbm import LGBMRankerMRR2
...
with timer("model fitting"):
    model = LGBMRankerMRR2(n_estimators=1600, num_leaves=62, n_jobs=-2)
    ...
    meta_val.to_csv(f"predictions/model_submit_{githash}_mrr2.csv", index=False)
    joblib.dump(model, "model_submit_mrr2.joblib")
