...
@click.option("--tail", type=int, default=None, help="Limit number of rows")
@click.option("--dst", type=str, default=None, help="Save results")
def main(src, tail, dst, limit):
    df_train = pd.read_csv(src, nrows=limit)
    if tail:
        df_train = df_train.tail(tail)
    # df = df[df["is_impression_the_same"] == True]
    results = []
    for col in tqdm(list(df_train.columns)[27:]):
        if df_train[col].dtype in [np.int, np.float] and col != "was_clicked":
