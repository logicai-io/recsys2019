...
def main(src, dst, limit):
    ...
    for col in tqdm(df.columns):
        if df[col].dtype in [np.int, np.float] and col != "was_clicked":
            ...
            df[col + "_rank_session"] = df.groupby("session_id")[col].rank("max", ascending=False)
            mrr_rank = mrr_fast(df, col + "_rank_session")
            df[col + "_rank_session"] = df.groupby("session_id")[col].rank("max", ascending=True)
            mrr_rank_rev = mrr_fast(df, col + "_rank_session")
            results.append((col + "_rank_session", max(mrr_rank, mrr_rank_rev)))
            df[col + "_rank"] = df.groupby("clickout_id")[col].rank("max", ascending=False)
            mrr_rank = mrr_fast(df, col + "_rank")
            df[col + "_rank"] = df.groupby("clickout_id")[col].rank("max", ascending=True)
            mrr_rank_rev = mrr_fast(df, col + "_rank")
