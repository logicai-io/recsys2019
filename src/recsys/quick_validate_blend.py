...
df_train["click_proba_1"] = model.predict(mat_train)
df_val["click_proba_1"] = model.predict(mat_val)
...
model.fit(
    ...
    group=group_lengths(df_train["clickout_id"]),
    verbose=True,
    eval_set=[(mat_val_2, df_val["was_clicked"])],
    eval_group=[group_lengths(df_val["clickout_id"])],
    eval_metric=mrr_metric,
    early_stopping_rounds=200,
)

df_train["click_proba_2"] = model.predict(mat_train_2)
df_val["click_proba_2"] = model.predict(mat_val_2)


# 0.6407197814774794
print(mrr_fast(df_val, "click_proba_2"))
