...
with timer("model fitting"):
    model = LGBMRanker(learning_rate=0.4, n_estimators=1600, num_leaves=62, n_jobs=-2)
    ...
    meta_val.to_csv(f"predictions/model_submit_{githash}_lr_0_4.csv", index=False)
    joblib.dump(model, "model_submit_lr_0_4.joblib")
