...
for model_hash in models:
    predictions.append(pd.read_csv(f"predictions/model_val_{model_hash}.csv"))
...
X = np.vstack([p["click_proba"] for p in predictions]).T
y = predictions[0]["was_clicked"].values
g = predictions[0]["clickout_id"].values

cutoff = X.shape[0] // 2

X_tr, X_te = X[:cutoff], X[cutoff:]
y_tr, y_te = y[:cutoff], y[cutoff:]
g_tr, g_te = g[:cutoff], g[cutoff:]

model = LGBMRanker(n_estimators=100, n_jobs=-2, verbose=3)
model.fit(X_tr, y_tr, group=group_lengths(g_tr),
    verbose=True,
    eval_set=[(X_tr, y_tr), (X_te, y_te)],
    eval_group=[group_lengths(g_tr), group_lengths(g_te)],
)
