...
with timer("reading data"):
    ...
    events = pd.read_csv("../../data/events_click_rev.csv").drop_duplicates()
    meta = pd.merge(meta, events, on=["user_id", "session_id", "step"], how="left")
    meta["clickout_step_rev"].fillna(2, inplace=True)
...
with timer("model fitting"):
    for sample_weight in [1.5, 2, 4]:
        ...
        model.fit(X_train, 
                  ...
                  sample_weight=np.where(meta_train["clickout_step_rev"]==1,sample_weight,1))
        ...
        meta_val.to_csv(f"predictions/model_val_{githash}_sample_weight_{sample_weight}.csv", index=False)
