...
def nn_fit_predict(xs, y_train):
    ...
    with tf.Session(graph=tf.Graph(), config=config) as sess:
        ...
        model.compile(loss="binary_crossentropy", optimizer=ks.optimizers.Adam(lr=0.01))
        for i in range(4):
            model.fit(x=X_train, y=y_train, batch_size=2 ** (10 + min(i, 3)), epochs=1, verbose=1, shuffle=False)
