...
def calc_mrr(val_data, val_preds, y_val_enc):
    clickout_ids = []
    preds = []
    clicks = []
...
def create_model(vect: SeqVectorizer):
    padding = vect.pad
    ...
    seq_actiontype_cat_input = ks.layers.Input(
        shape=[padding, len(vect.seq_vectorizers["action type cat"].idx)], name="action_type_cat"
    )
    seq_clickout_cat_input = ks.layers.Input(
        shape=[padding, len(vect.seq_vectorizers["clickout item cat"].idx)], name="clickout_item_cat"
    )
    seq_int_img_cat_input = ks.layers.Input(
        shape=[padding, len(vect.seq_vectorizers["interaction item image cat"].idx)], name="interaction_item_image_cat"
    )
    seq_int_info_cat_input = ks.layers.Input(
        shape=[padding, len(vect.seq_vectorizers["interaction item info cat"].idx)], name="interaction_item_info_cat"
    )
    seq_int_rating_cat_input = ks.layers.Input(
        shape=[padding, len(vect.seq_vectorizers["interaction item rating cat"].idx)],
        name="interaction_item_rating_cat",
    )
    seq_int_deals_cat_input = ks.layers.Input(
        shape=[padding, len(vect.seq_vectorizers["interaction item deals cat"].idx)], name="interaction_item_deals_cat"
    )
    seq_change_sort_cat_input = ks.layers.Input(
        shape=[padding, len(vect.seq_vectorizers["change of sort order cat"].idx)], name="change_of_sort_order_cat"
    )

    seq_clickout_num_input = ks.layers.Input(shape=[padding, 1], name="clickout_item_num")
    seq_timestamp_diff_num_input = ks.layers.Input(shape=[padding, 1], name="timestamp_diff_num")
    seq_count_action_num_input = ks.layers.Input(shape=[padding, 1], name="count_action_num")
