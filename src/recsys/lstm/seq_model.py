...
def concat_oof_predictions():
    ...
    val_path_a = "../../../data/lstm/seq_user_session_all_hash_1.ndjson.gzip"
    val_preds_df_a = oof_seq_predictions(train_path_a, val_path_a)

    train_path_b = "../../../data/lstm/seq_user_session_train_hash_1.ndjson.gzip"
    val_path_b = "../../../data/lstm/seq_user_session_all_hash_0.ndjson.gzip"
    val_preds_df_b = oof_seq_predictions(train_path_b, val_path_b)

    all_preds_df = pd.concat([val_preds_df_a, val_preds_df_b], axis=0).drop_duplicates(
        ["user_id", "session_id", "step", "item_id"])
    return all_preds_df


def oof_seq_predictions(train_path, val_path):
    ...
    model.fit(
        train_data_tr, y_train_enc, validation_data=(val_data_tr, y_val_enc), epochs=5, batch_size=256, shuffle=True
    )
...
if __name__ == "__main__":
    ...
    """
    ALL
    MRR TRAIN 0.6457 VAL 0.6440
    
    1_000_000
    MRR TRAIN 0.6304 VAL 0.6214
    MRR TRAIN 0.6381 VAL 0.6329
    MRR TRAIN 0.6314 VAL 0.6255
    MRR TRAIN 0.6322 VAL 0.6264
    """
