...
min_price_by_hotel = df_prices.groupby("item_id")["price"].min().reset_index().rename(columns={"price": "min_price_hist"})
max_price_by_hotel = df_prices.groupby("item_id")["price"].max().reset_index().rename(columns={"price": "max_price_hist"})
...
df_prices["price_relative_to_min"] = df_prices["price"] / df_prices["min_price_hist"]
df_prices["price_range"] = df_prices["max_price_hist"] - df_prices["min_price_hist"]
df_prices["price_range_div"] = df_prices["max_price_hist"] / df_prices["min_price_hist"]
