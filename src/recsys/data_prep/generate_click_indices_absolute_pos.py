    ...
    for session in user:
        if len(session) > 1:
            for ((t1, c1), (t2, c2)) in zip(session[:-1], session[1:]):
                if t2 - t1 <= 120:
                    records.append((c1, c2, t2 - t1))

df = pd.DataFrame.from_records(records)
df.columns = ["rank_1", "rank_2", "diff_time"]
df["diff_time_grouped"] = df["diff_time"].map(group_time)
df["one"] = 1

diff_time_pivot = df.pivot_table(values="one", index=["diff_time_grouped", "rank_1"], columns="rank_2", aggfunc=len)
...
joblib.dump(d, "../../../data/click_probs_by_index.joblib")
