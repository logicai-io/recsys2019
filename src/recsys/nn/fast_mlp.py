...
class FastMLPRegressor(BaseEstimator, RegressorMixin):
    ACTFUNC_MAP = {"logistic": (logistic, 3), "tanh": (np.tanh, 2), "relu": (relu, 1)}

    LOSS_MAP = {"squared": 1, "huber": 2}
    ...
    def fit(self, X, y, init=True):
        ...
        for epoch in range(self.nepochs):
            ...
            if self.batchsize == 1:
                if self.nhidden2:
                    fit_epoch_2_layers(
                        ...
                        self.W2,
                        self.V,
                        self.ACTFUNC_MAP[self.actfunc][1],
                        eta,
                        self.dropout,
                        self.lambd,
                        self.LOSS_MAP[self.loss],
                        self.huber_sigma,
                    )
                else:
                    fit_epoch(
            ...
            if self.evalfunc != None and epoch % 5 == 4:
                self.evalfunc(self)
    ...
    def predict_all_epochs(self, X):
        X = sp.csr_matrix(X).astype(np.float32)
        return np.hstack(
            [self.ACTFUNC_MAP[self.actfunc][0](X.dot(W)).dot(V) for W, V in zip(self.W_history, self.V_history)]
        )
